...
package ybprjgit.copy_of_usecustomjavacode_0_1;
...
/**
 * Job: Copy_of_useCustomJavaCode Purpose: <br>
 * Description: <br>
 * 
 * @author Lee, Yongbum
 ...
 * @status
 */
public class Copy_of_useCustomJavaCode implements TalendJob {
	static {
		System.setProperty("TalendJob.log", "Copy_of_useCustomJavaCode.log");
	}

	private static org.apache.logging.log4j.Logger log = org.apache.logging.log4j.LogManager
			.getLogger(Copy_of_useCustomJavaCode.class);
	...
	public class ContextProperties extends PropertiesWithType {
		...
		public void synchronizeContext() {
			...
			if (UID1 != null) {
				...
				this.setProperty("UID1", UID1.toString());
	...
	private final String jobName = "Copy_of_useCustomJavaCode";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					Copy_of_useCustomJavaCode.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(Copy_of_useCustomJavaCode.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class enRich2Struct implements routines.system.IPersistableRow<enRich2Struct> {
		final static byte[] commonByteArrayLock_YBPRJGIT_Copy_of_useCustomJavaCode = new byte[0];
		static byte[] commonByteArray_YBPRJGIT_Copy_of_useCustomJavaCode = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_YBPRJGIT_Copy_of_useCustomJavaCode.length) {
					if (length < 1024 && commonByteArray_YBPRJGIT_Copy_of_useCustomJavaCode.length == 0) {
						commonByteArray_YBPRJGIT_Copy_of_useCustomJavaCode = new byte[1024];
					} else {
						commonByteArray_YBPRJGIT_Copy_of_useCustomJavaCode = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_YBPRJGIT_Copy_of_useCustomJavaCode, 0, length);
				strReturn = new String(commonByteArray_YBPRJGIT_Copy_of_useCustomJavaCode, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_YBPRJGIT_Copy_of_useCustomJavaCode, 0, length);
				strReturn = new String(commonByteArray_YBPRJGIT_Copy_of_useCustomJavaCode, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_YBPRJGIT_Copy_of_useCustomJavaCode) {
		 ...
		 * Compare keys
		 */
		public int compareTo(enRich2Struct other) {
	...
	public void tFileInputDelimited_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		org.slf4j.MDC.put("_subJobPid", "2Rq3wB_" + subJobPidCounter.getAndIncrement());
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				enRich2Struct enRich2 = new enRich2Struct();
				...
				runStat.updateStatAndLog(execStat, enableLogStash, resourceMap, iterateId, 0, 0, "enRich2");
...
// # Outputs initialization
				int count_enRich2_tMap_2 = 0;

				enRich2Struct enRich2_tmp = new enRich2Struct();
// ###############################
				...
				context.UID1 = Numeric.random(1, 100) + "";
...
// Start of branch "row1"
						if (row1 != null) {
							...
							if (runStat.update(execStat, enableLogStash, iterateId, 1, 1
									...
									, "row1", "tFileInputDelimited_1", "customer", "tFileInputDelimited", "tJava_1",
									"tJava_1", "tJava"
							...
							if (runStat.update(execStat, enableLogStash, iterateId, 1, 1
									...
									, "row4", "tJava_1", "tJava_1", "tJava", "tMap_2", "tMap_2", "tMap"
...
// # Output table : 'enRich2'
								count_enRich2_tMap_2++;

								enRich2_tmp.ID = context.UID1
										+ String.format("%10s", Numeric.sequence("s1", 1, 1) + "").replace(' ', '0');
								;
								enRich2_tmp.Firstname = row4.Firstname;
								enRich2_tmp.Lastname = row4.Lastname;
								enRich2_tmp.Address = row4.Address;
								enRich2_tmp.City = row4.City;
								enRich2_tmp.Zipcode = row4.Zipcode;
								enRich2_tmp.State = row4.State;
								enRich2_tmp.Country = row4.Country;
								enRich2 = enRich2_tmp;
								log.debug("tMap_2 - Outputting the record " + count_enRich2_tMap_2
										+ " of the output table 'enRich2'.");
...
// Start of branch "enRich2"
							if (enRich2 != null) {
								...
								if (runStat.update(execStat, enableLogStash, iterateId, 1, 1
										...
										, "enRich2", "tMap_2", "tMap_2", "tMap", "tLogRow_2", "tLogRow_2", "tLogRow"
								...
								if (log.isTraceEnabled()) {
									log.trace("enRich2 - " + (enRich2 == null ? "" : enRich2.toLogString()));
								}
...
///////////////////////		
								...
								if (enRich2.ID != null) { //
									...
									strBuffer_tLogRow_2.append(String.valueOf(enRich2.ID));

								} //

								strBuffer_tLogRow_2.append("|");

								if (enRich2.Firstname != null) { //

									strBuffer_tLogRow_2.append(String.valueOf(enRich2.Firstname));

								} //

								strBuffer_tLogRow_2.append("|");

								if (enRich2.Lastname != null) { //

									strBuffer_tLogRow_2.append(String.valueOf(enRich2.Lastname));

								} //

								strBuffer_tLogRow_2.append("|");

								if (enRich2.Address != null) { //

									strBuffer_tLogRow_2.append(String.valueOf(enRich2.Address));

								} //

								strBuffer_tLogRow_2.append("|");

								if (enRich2.City != null) { //

									strBuffer_tLogRow_2.append(String.valueOf(enRich2.City));

								} //

								strBuffer_tLogRow_2.append("|");

								if (enRich2.Zipcode != null) { //

									strBuffer_tLogRow_2.append(String.valueOf(enRich2.Zipcode));

								} //

								strBuffer_tLogRow_2.append("|");

								if (enRich2.State != null) { //

									strBuffer_tLogRow_2.append(String.valueOf(enRich2.State));

								} //

								strBuffer_tLogRow_2.append("|");

								if (enRich2.Country != null) { //

									strBuffer_tLogRow_2.append(String.valueOf(enRich2.Country));
...
///////////////////////    			
				...
				if (runStat.updateStatAndLog(execStat, enableLogStash, resourceMap, iterateId, "row1", 2, 0,
						"tFileInputDelimited_1", "customer", "tFileInputDelimited", "tJava_1", "tJava_1", "tJava",
						"output")) {
					talendJobLogProcess(globalMap);
				}
...
// ###############################      
				log.debug("tMap_2 - Written records count in the table 'enRich2': " + count_enRich2_tMap_2 + ".");

				if (runStat.updateStatAndLog(execStat, enableLogStash, resourceMap, iterateId, "row4", 2, 0, "tJava_1",
						"tJava_1", "tJava", "tMap_2", "tMap_2", "tMap", "output")) {
					talendJobLogProcess(globalMap);
				}
...
///////////////////////    			
				...
				if (runStat.updateStatAndLog(execStat, enableLogStash, resourceMap, iterateId, "enRich2", 2, 0,
	...
	public static void main(String[] args) {
		final Copy_of_useCustomJavaCode Copy_of_useCustomJavaCodeClass = new Copy_of_useCustomJavaCode();

		int exitCode = Copy_of_useCustomJavaCodeClass.runJobInTOS(args);
		if (exitCode == 0) {
			log.info("TalendJob: 'Copy_of_useCustomJavaCode' - Done.");
		}
	...
	public int runJobInTOS(String[] args) {
		...
		getjobInfo();
		log.info("TalendJob: 'Copy_of_useCustomJavaCode' - Start.");
		...
		org.slf4j.MDC.put("_jobRepositoryId", "_Mc6cAPAYEe-LXeArAikIEA");
		org.slf4j.MDC.put("_compiledAtTimestamp", "2025-02-21T05:56:00.331883500Z");
		...
		try {
			...
			java.io.InputStream inContext = Copy_of_useCustomJavaCode.class.getClassLoader().getResourceAsStream(
					"ybprjgit/copy_of_usecustomjavacode_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = Copy_of_useCustomJavaCode.class.getClassLoader()
			...
			class ContextProcessing {
				private void processContext_0() {
					context.setContextType("UID1", "id_String");
					if (context.getStringValue("UID1") == null) {
						context.UID1 = null;
					} else {
						context.UID1 = (String) context.getProperty("UID1");
					}
				}
		...
		if (parentContextMap != null && !parentContextMap.isEmpty()) {
			if (parentContextMap.containsKey("UID1")) {
				context.UID1 = (String) parentContextMap.get("UID1");
			}
		}
		...
		log.info("TalendJob: 'Copy_of_useCustomJavaCode' - Started.");
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory)
					+ " bytes memory increase when running : Copy_of_useCustomJavaCode");
		}
		if (enableLogStash) {
		...
		log.info(
				"TalendJob: 'Copy_of_useCustomJavaCode' - Finished - status: " + status + " returnCode: " + returnCode);
...
/************************************************************************************************
 * 112231 characters generated by Talend Real-time Big Data Platform on the
 * 2025년 2월 21일 오후 2시 56분 0초 KST
 ************************************************************************************************/